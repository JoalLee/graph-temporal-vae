...
def load_external_heldout_mask(
    ...
    *,
    ...
    """Load and validate an externally supplied full-timeline HO mask.
    ...
    semantically intact. This loader does not claim that the artifact was
    generated by the current anchor-constrained sampler. The overlap is
    reported so a changed QC source cannot silently look like the original
    benchmark; ``WindowedTimeSeriesDataset`` intersects a fixed mask with the
    actually observed cells when constructing training/evaluation masks.
    """
    if not mask_path:
    ...
    if censored_mask is None:
        censored_mask = np.zeros_like(mask, dtype=bool)
    else:
        censored_mask = np.asarray(censored_mask, dtype=bool)
        if censored_mask.shape != mask.shape:
            raise ValueError(
                "censored_mask shape does not match external held-out mask: "
                f"censored={censored_mask.shape}, mask={mask.shape}"
            )
    overlap = mask & ~observed_mask & ~censored_mask
    censored_overlap = mask & censored_mask
    diagnostics = {
        ...
        "censored_overlap_cells": int(censored_overlap.sum()),
    }
    ...
    *, trainer, config, model_kwargs, target_cols, aux_cols, preprocessing,
    ...
    return {
        ...
        "selection_mask_protocol": (
            "shared_full_external_ho"
            if selection_full is not None and config.selection_mask_path
            else (
        ...
        "selection_mask_source": (
            "external" if config.selection_mask_path else "generated"
        ),
        "selection_mask_generation_mode": (
            None if config.selection_mask_path else config.selection_mask_mode
        ),
        "schema": {
            "target_cols": list(target_cols),
...
def train_from_config(
    ...
    *,
    ...
    censor_report = censoring_report(
        state_full, data_schema, censoring, marker_mask=marker_mask
    )
    ...
    if full_data_validation and config.shared_full_heldout_mask:
        ...
        if config.selection_mask_path:
            selection_full, selection_mask_diagnostics = _load_external_heldout_mask(
                ...
                censored_mask=censor_full,
            )
            if (
                selection_mask_diagnostics["natural_missing_overlap_cells"]
                or selection_mask_diagnostics["censored_overlap_cells"]
            ):
                print(
